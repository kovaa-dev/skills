...
codex plugin add kovaa-agent-skills@kovaa
```
...
This repository is also a self-hosted Claude Code plugin marketplace. In Claude Code, add the marketplace and install the bundle:

```text
/plugin marketplace add kovaa-dev/skills
/plugin install kovaa-agent-skills@kovaa
```

The plugin installs both independent skills. Restart the active Claude Code conversation after installation or update so the skills can be rediscovered.

A public listing on [claude.com/plugins](https://claude.com/plugins) requires a separate submission to Anthropic's `claude-community` marketplace and their validation and safety review. The self-hosted installation above does not depend on directory acceptance.
...
The current repository and plugin version is `v0.3.1`. <!-- x-release-please-version -->
